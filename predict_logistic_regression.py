...
category_model=joblib.load("models/logistic_regression_category_model.pkl")
priority_model=joblib.load("models/logistic_regression_priority_model.pkl")
...
def predict_ticket(subject, description):
    ...
    cate_pred=category_model.predict(vec)[0]
    prio_pred=priority_model.predict(vec)[0]
    cate_prob=category_model.predict_proba(vec)[0]
    prio_prob=priority_model.predict_proba(vec)[0]
    cate_conf=float(max(cate_prob))
    prio_conf=float(max(prio_prob))
    category=category_encoder.inverse_transform([cate_pred])[0]
